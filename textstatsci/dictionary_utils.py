...
def _get_default_dict_path(lang="en"):
    ...
    return f'resources/{lang}/custom_dict.json'


def _get_user_dict_path(lang="en"):
    ...
    os.makedirs(lang_dir, exist_ok=True)  # Create lang dir if it doesn't exist
    return os.path.join(lang_dir, 'custom_dict.json')


def load_custom_syllable_dict(lang="en"):
    ...
    """
    user_dict_path = _get_user_dict_path(lang)
    default_dict_path = _get_default_dict_path(lang)
    loaded_dict = {}
...
def overwrite_custom_dict(file_path, lang="en"):
...
def add_term_to_custom_dict(word, syllable_count, lang="en"):
    ...
    user_dict_path = _get_user_dict_path(lang)
    current_dict = load_custom_syllable_dict(lang)  # Load existing dict (user or default)
...
def add_terms_from_file(file_path, lang="en"):
    ...
    try:
        ...
        current_dict = load_custom_syllable_dict(lang)
...
def revert_custom_dict_to_default(lang="en"):
    ...
    customizations made by the user for the specified language.
    """
    user_dict_path = _get_user_dict_path(lang)
    default_dict_path = _get_default_dict_path(lang)
    ...
    try:
        ...
        resource_path = _get_default_dict_path(lang)
        ...
        print(
            f"Custom dictionary for language '{lang}' reverted to the default package dictionary. "
